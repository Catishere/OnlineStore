...
describe('Services', () => {
    ...
    describe('Authentication Service', () => {
        ...
        it(`Should properly call 'register'`, async () => {
            const loginPostSpy = spyOn<any>(authServ, 'loginPost').and.returnValue(Promise.resolve());
            const dummyData = { firstName: 'Testy', lastName: 'Testington', email: 'Dummy', password: 'Data' };
        ...
        describe(`private`, () => {
            it(`Should properly call 'getStorageUser'`, () => {
                ...
                expect((provider as any).getStorageUser(mockInput)).toEqual({
                    id: `1`,
                    name: mockInput.body.firstName + ' ' + mockInput.body.lastName,
                    email: mockInput.body.email,
                    firstName: mockInput.body.firstName,
                    lastName: mockInput.body.lastName,
                    picture: '',
                });
            });
    ...
    describe(`User Service`, () => {
        ...
        it(`Should properly get 'initials'`, () => {
            ...
            currentUserSpy.and.returnValue({ firstName: '' });
            expect(userServ.initials).toEqual('');
            currentUserSpy.and.returnValue({ firstName: '', lastName: '' });
            expect(userServ.initials).toEqual('');
            currentUserSpy.and.returnValue({ firstName: '', lastName: 'g' });
            expect(userServ.initials).toEqual('g');
            currentUserSpy.and.returnValue({ firstName: 'h3ll0' });
            expect(userServ.initials).toEqual('h');
            currentUserSpy.and.returnValue({ firstName: 'h3ll0', lastName: 'g' });
            expect(userServ.initials).toEqual('hg');
            currentUserSpy.and.returnValue({ firstName: 'h3ll0', lastName: '' });
            expect(userServ.initials).toEqual('h');
        });
